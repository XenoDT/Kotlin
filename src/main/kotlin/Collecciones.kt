...
//    }
    ...
    fun filterByZipCode(students: List<Student>, zipCode: Int): List<Student> =
        students.filter { student -> student.zipCode == zipCode }.map { student -> student }
...
//        numbers.min()
    ...
    fun junction(numbers1: List<Double> , numbers2: List<Double>): List<Double> =
        numbers1.intersect(numbers2).toList()

    fun preciodePedidos(orders: List<Order>): List<Double> =
        orders.map { order -> order.price }

    fun precioTotal(orders: List<Order>): Double =
        orders.map { order -> order.price }.reduce { acc, d -> acc + d }

    fun getItems(orders: List<Order>): List<OrderItem> =
